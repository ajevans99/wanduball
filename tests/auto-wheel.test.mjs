...
test("auto sequence visits only locked pools in position order and ends after points", () => {
    ...
    while (nextWheel(state)) {
        ...
        assert.ok(commands.length <= 23);
        state = transition(state, command, () => 0, now += 10_000);
    }
    assert.deepEqual(commands, [...Array(10).fill("QB"), ...Array(10).fill("TE"), "coin", "rule", "points"]);
    ...
    assert.equal(state.changes[0].applied, false);
    assert.equal(state.week, 1);
});

test("auto resumes a partially completed round and does not reroll pending decisions", () => {
    const state = initialState();
    state.pending.duration = "Permanent";
    assert.deepEqual(nextWheel(state)?.command, { type: "rule" });
    state.pending.ruleId = state.rules.find(rule => rule.duration === "Permanent").id;
    assert.deepEqual(nextWheel(state)?.command, { type: "points" });
    state.rules = [];
    assert.equal(nextWheel(state), null);
});
