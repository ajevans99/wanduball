...
function Game({ roomId }: {
    ...
    async function importPlayers() {
        ...
        try {
            const data = await requestJson<Omit<Extract<Command, {
                type: "import";
            }>, "type">>(`/api/sleeper?leagueId=${encodeURIComponent(leagueId)}&season=${importSeason}&week=${importWeek}&statsSeason=${statsSeason}&ranking=ppr`);
    ...
    <div className="workspace">
      ...
      <main>
        ...
        {tab === "setup" && <>
          <section className="panel import-panel"><div><h3><ExternalLink size={18}/> Straight from the Sleeper swamp</h3><p>Season-to-date PPR points, ranked by position. Earlier seasons use full-season totals. Exclude injuries and byes manually.</p></div><form className="import-form" onSubmit={e => { e.preventDefault(); void importPlayers(); }}><label className="wide">Sleeper league ID<input required pattern="[0-9]+" placeholder="e.g. 1234567890123456789" value={leagueId} onChange={e => setLeagueId(e.target.value)}/></label><label>League season<input type="number" min="2020" max="2100" value={importSeason} onChange={e => setImportSeason(Number(e.target.value))} required/></label><label>Week<input type="number" min="1" max="18" value={importWeek} onChange={e => setImportWeek(Number(e.target.value))} required/></label><label>Stats season<input type="number" min="2020" max={importSeason} value={statsSeason} onChange={e => setStatsSeason(Number(e.target.value))} required/></label><button className="button dark" disabled={!canEdit || busy || Boolean(game.locked.length)}>{busy ? <LoaderCircle className="loading" size={16}/> : <ArrowDownToLine size={16}/>}Pull the players</button></form><small>Week 1? Use the previous season for rankings. Import is read-only; nothing gets changed in Sleeper.</small></section>
